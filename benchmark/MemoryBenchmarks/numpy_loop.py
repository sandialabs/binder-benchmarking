...
def numpy_python_loop():
    for _ in range(N):
        gamma = pybind11_gravitation_ecef(WhiteHouse_mean_prop["prior"]["position"]) #not really good to have a mix of generating results from different types.  However, no time and this is what is used in the example
...
def main():
    ...
    warmup_base(warmup_cycles,numpy_python_loop)
    timeit.timeit(numpy_python_loop,number=benchmark_cycles)
